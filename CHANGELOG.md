...
- The built-in `browseract.extract_account_facts` handler now executes through the shared tool plane too, resolves BrowserAct-backed account facts from a scoped connector binding, and can optionally call a configured live BrowserAct run URL before handing structured discovery output to artifact persistence.
