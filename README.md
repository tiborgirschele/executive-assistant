...
- `/v1/skills` now projects a first-class skill catalog layer over task contracts, so executive capabilities like `meeting_prep` can persist product-facing metadata (`memory_reads`, `memory_writes`, authority/tool/human/provider policy, evaluation cases, and workflow template selection) without introducing a second storage system; `SKILLS.md` tracks the initial catalog, `meeting_prep` now has focused plus API-smoke coverage, and `GET /v1/skills?provider_hint=BrowserAct` filters the catalog by LTD-backed provider hints
