...
- `/v1/skills` now projects a first-class skill catalog layer over task contracts, so executive capabilities like `meeting_prep` and `ltd_inventory_refresh` can persist product-facing metadata (`memory_reads`, `memory_writes`, authority/tool/human/provider policy, evaluation cases, and workflow template selection) without introducing a second storage system; `SKILLS.md` tracks the initial catalog, those first guarded slices now cover both a memory-backed executive brief and a BrowserAct-backed LTD inventory refresh, and `GET /v1/skills?provider_hint=BrowserAct` filters the catalog by LTD-backed provider hints
...
- Task contracts can now also set `budget_policy_json.artifact_output_template=evidence_pack`, causing `step_input_prepare` to emit structured `claims`, `evidence_refs`, `open_questions`, and `confidence` metadata that flows into `step_artifact_save` and persists as a first-class evidence envelope instead of only plain text
