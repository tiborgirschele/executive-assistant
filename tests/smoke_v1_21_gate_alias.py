...
def test_v121_gate_alias_script_and_docs() -> None:
    ...
    alias_script_v122 = ROOT / "scripts/run_v122_smoke.sh"
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    script_src = alias_script.read_text(encoding="utf-8")
    script_src_v122 = alias_script_v122.read_text(encoding="utf-8")
    assert alias_script.exists()
    assert alias_script_v122.exists()
    assert "run_v120_smoke.sh" in script_src
    assert "run_v120_smoke.sh" in script_src_v122
