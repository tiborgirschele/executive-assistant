...
def test_synthetic_preview_outcome_source() -> None:
    ...
    try:
        res = dispatch_skill_operation(
            ...
            tenant="chat_100284",
            chat_id=123,
            payload={"notes": "Warm and concise"},
        )
    finally:
    ...
    row = calls[-1]
    assert str(row.get("source") or "") == "synthetic_preview"
    assert str(row.get("outcome_status") or "") == "synthetic_preview"
    assert int(row.get("score_delta") if row.get("score_delta") is not None else -999) == 0
    _pass("v1.22 synthetic preview outcome source")


if __name__ == "__main__":
    test_synthetic_preview_outcome_source()
