...
def trigger_mum_brain(db_conn, e_msg, fallback="telegram_text", failure_class="system_error", intent="unknown"):
    ...
    if db_conn:
        ...
        try:
            ...
            if hasattr(db_conn, 'execute'): db_conn.execute(sql, (intent, failure_class, 'inline_fallback', cid, ctx))
            elif hasattr(db_conn, 'cursor'):
                with db_conn.cursor() as cur: cur.execute(sql, (intent, failure_class, 'inline_fallback', cid, ctx))
