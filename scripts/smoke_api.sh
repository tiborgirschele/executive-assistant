...
SESSION_HUMAN_NONE_PROJECTION_FIELDS="$(python3 -c "import json,sys; body=json.loads(sys.stdin.read() or '{}'); wanted=['${HUMAN_OWNERLESS_ID}','${HUMAN_OWNERLESS_NEWER_ID}']; current_blocked='${HUMAN_TASK_ID}'; tasks=body.get('human_tasks') or []; history=body.get('human_task_assignment_history') or []; wanted_tasks=[row for row in tasks if (row or {}).get('human_task_id') in wanted]; wanted_history=[row for row in history if (row or {}).get('human_task_id') in wanted]; current_only=all((row or {}).get('human_task_id') != current_blocked for row in tasks); history_prefix='|'.join((row or {}).get('human_task_id','') for row in history[:3]); print('{}|{}|{}|{}'.format('|'.join((row or {}).get('human_task_id','') for row in wanted_tasks[:2]), '|'.join((row or {}).get('human_task_id','') for row in wanted_history[:2]), current_only, history_prefix))" <<<"${SESSION_HUMAN_NONE_PROJECTION_JSON}")"
if [[ "${SESSION_HUMAN_NONE_PROJECTION_FIELDS}" != "${HUMAN_OWNERLESS_ID}|${HUMAN_OWNERLESS_NEWER_ID}|${HUMAN_OWNERLESS_ID}|${HUMAN_OWNERLESS_NEWER_ID}|True|${HUMAN_TASK_ID}|${HUMAN_OWNERLESS_ID}|${HUMAN_OWNERLESS_NEWER_ID}" ]]; then
  echo "expected session detail human_task_assignment_source=none projection to keep current ownerless rows isolated while preserving oldest-first empty-source history under mixed-source churn; got ${SESSION_HUMAN_NONE_PROJECTION_FIELDS}" >&2
