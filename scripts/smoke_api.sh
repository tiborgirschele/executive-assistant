...
if [[ "${REVIEW_PLAN_FIELDS}" != "4|human_task|human|draft|operator|communications_reviewer|high|45|2700|1|0|True|manager_review|send_on_behalf_review" ]]; then
...
if [[ "${HUMAN_REWRITE_SESSION_FIELDS}" != "awaiting_human|True|True|True|True|human|draft|operator|fail|2700|1|0|high|True|manager_review|send_on_behalf_review|Executive-facing rewrite needs human judgment before finalization.|tone|assigned|operator-specialist|auto_preselected|True|orchestrator:auto_preselected|human_task_created,human_task_assigned" ]]; then
