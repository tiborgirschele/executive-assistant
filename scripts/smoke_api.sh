...
curl -fsS -X POST "${BASE}/v1/tasks/contracts" "${AUTH_ARGS[@]}" -H 'content-type: application/json' \
  -d '{"task_key":"rewrite_review","deliverable_type":"rewrite_note","default_risk_class":"low","default_approval_class":"none","allowed_tools":["artifact_repository"],"evidence_requirements":["stakeholder_context"],"memory_write_policy":"reviewed_only","budget_policy_json":{"class":"low","human_review_role":"communications_reviewer","human_review_task_type":"communications_review","human_review_brief":"Review the rewrite before finalizing it.","human_review_priority":"high","human_review_sla_minutes":45,"human_review_desired_output_json":{"format":"review_packet","escalation_policy":"manager_review"}}}' >/dev/null
...
REVIEW_PLAN_FIELDS="$(python3 -c "import json,sys; body=json.loads(sys.stdin.read() or '{}'); steps=body.get('plan',{}).get('steps') or []; review=(steps[2] if len(steps) > 2 else {}); print('{}|{}|{}|{}|{}|{}|{}'.format(len(steps), review.get('step_kind',''), review.get('role_required',''), review.get('priority',''), review.get('sla_minutes',''), (review.get('desired_output_json') or {}).get('escalation_policy',''), ','.join((steps[3] or {}).get('depends_on') or []) if len(steps) > 3 else ''))" <<<"${REVIEW_PLAN_JSON}")"
if [[ "${REVIEW_PLAN_FIELDS}" != "4|human_task|communications_reviewer|high|45|manager_review|step_human_review" ]]; then
...
curl -fsS -X POST "${BASE}/v1/tasks/contracts" "${AUTH_ARGS[@]}" -H 'content-type: application/json' \
  -d '{"task_key":"rewrite_text","deliverable_type":"rewrite_note","default_risk_class":"low","default_approval_class":"none","allowed_tools":["artifact_repository"],"evidence_requirements":["stakeholder_context"],"memory_write_policy":"reviewed_only","budget_policy_json":{"class":"low","human_review_role":"communications_reviewer","human_review_task_type":"communications_review","human_review_brief":"Review the rewrite before finalizing it.","human_review_priority":"high","human_review_sla_minutes":45,"human_review_desired_output_json":{"format":"review_packet","escalation_policy":"manager_review"}}}' >/dev/null
...
HUMAN_REWRITE_SESSION_FIELDS="$(python3 -c "import json,sys; body=json.loads(sys.stdin.read() or '{}'); tasks=body.get('human_tasks') or []; queues=body.get('queue_items') or []; steps=body.get('steps') or []; review=next((row for row in tasks if (row or {}).get('human_task_id') == '${HUMAN_REWRITE_TASK_ID}'), {}); print('{}|{}|{}|{}|{}|{}|{}|{}'.format(body.get('status',''), len(steps) == 4, len(queues) == 3 and all((q or {}).get('state','') == 'done' for q in queues), bool(review.get('human_task_id','')) and review.get('status') == 'pending', any((row or {}).get('step_id') and (row or {}).get('input_json',{}).get('plan_step_key') == 'step_human_review' and (row or {}).get('state') == 'waiting_human' for row in steps), review.get('priority',''), bool(review.get('sla_due_at','')), (review.get('desired_output_json') or {}).get('escalation_policy','')))" <<<"${HUMAN_REWRITE_SESSION_JSON}")"
if [[ "${HUMAN_REWRITE_SESSION_FIELDS}" != "awaiting_human|True|True|True|True|high|True|manager_review" ]]; then
