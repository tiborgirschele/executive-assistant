...
if [[ "${TASK_EXECUTE_SESSION_FIELDS}" != "stakeholder_briefing|completed|3|stakeholder_briefing|stakeholder_briefing|stakeholder_briefing|True|system|observe|30|tool|fail|system|30|Board context and stakeholder sensitivities.|artifact://${TASK_EXECUTE_ARTIFACT_ID}|${PRINCIPAL_ID}|True|email|alex-exec|thread:board-prep,memory:item:stakeholder-brief" ]]; then
  echo "expected generic task execution session to retain compiled step semantics, honest single-dependency parent links, retry/timeout budgets, explicit artifact ownership fields, and structured input/context refs; got ${TASK_EXECUTE_SESSION_FIELDS}" >&2
