...
TOOL_FIELDS="$(python3 -c "import json,sys; rows=json.loads(sys.stdin.read() or '[]'); names={row.get('tool_name','') for row in rows}; print('{}|{}|{}|{}|{}'.format('artifact_repository' in names, 'browseract.extract_account_facts' in names, 'browseract.extract_account_inventory' in names, 'connector.dispatch' in names, 'email.send' in names))" <<<"${TOOLS_JSON}")"
if [[ "${TOOL_FIELDS}" != "True|True|True|True|True" ]]; then
  echo "expected tool registry to expose builtin artifact_repository, builtin browseract.extract_account_facts, builtin browseract.extract_account_inventory, builtin connector.dispatch, and upserted email.send; got ${TOOL_FIELDS}" >&2
  echo "${TOOLS_JSON}" >&2
  fail 12 "policy contract mismatch"
fi
...
BROWSERACT_INVENTORY_JSON="$(curl -fsS -X POST "${BASE}/v1/tools/execute" "${AUTH_ARGS[@]}" -H 'content-type: application/json' \
  "${PRINCIPAL_ARGS[@]}" \
  -d "{\"tool_name\":\"browseract.extract_account_inventory\",\"action_kind\":\"account.extract_inventory\",\"payload_json\":{\"binding_id\":\"${BROWSERACT_BINDING_ID}\",\"service_names\":[\"BrowserAct\",\"Teable\",\"UnknownService\"],\"requested_fields\":[\"tier\",\"account_email\",\"status\"]}}")"
BROWSERACT_INVENTORY_FIELDS="$(python3 -c "import json,sys; body=json.loads(sys.stdin.read() or '{}'); out=body.get('output_json') or {}; services=out.get('services_json') or []; receipt=body.get('receipt_json') or {}; print('{}|{}|{}|{}|{}|{}|{}'.format(body.get('tool_name',''), ','.join(out.get('service_names') or []), ','.join(out.get('missing_services') or []), (services[1].get('plan_tier','') if len(services) > 1 else ''), (services[2].get('discovery_status','') if len(services) > 2 else ''), receipt.get('handler_key',''), receipt.get('invocation_contract','')))" <<<"${BROWSERACT_INVENTORY_JSON}")"
if [[ "${BROWSERACT_INVENTORY_FIELDS}" != "browseract.extract_account_inventory|BrowserAct,Teable,UnknownService|UnknownService|License Tier 4|missing|browseract.extract_account_inventory|tool.v1" ]]; then
...
SKILL_FILTER_JSON="$(curl -fsS "${BASE}/v1/skills?limit=10&provider_hint=browseract" "${AUTH_ARGS[@]}")"
...
SKILL_FIELDS="$(python3 -c "import json,sys; body=json.loads(sys.argv[1]); listed=json.loads(sys.argv[2]); filtered=json.loads(sys.argv[3]); fetched=json.loads(sys.argv[4]); compiled=json.loads(sys.argv[5]); steps=compiled.get('plan',{}).get('steps') or []; print('{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}'.format(body.get('skill_key',''), body.get('workflow_template',''), ','.join(body.get('memory_reads') or []), ','.join(body.get('memory_writes') or []), ','.join((body.get('provider_hints_json') or {}).get('primary') or []), any(row.get('skill_key') == 'meeting_prep' for row in listed), [row.get('skill_key') for row in filtered], fetched.get('name',''), (fetched.get('authority_profile_json') or {}).get('authority_class',''), ','.join((fetched.get('provider_hints_json') or {}).get('research') or []), compiled.get('skill_key',''), len(steps), ','.join(step.get('step_key','') for step in steps)))" "${SKILL_JSON}" "${SKILL_LIST_JSON}" "${SKILL_FILTER_JSON}" "${SKILL_FETCH_JSON}" "${SKILL_PLAN_JSON}")"
if [[ "${SKILL_FIELDS}" != "meeting_prep|artifact_then_memory_candidate|stakeholders,commitments,decision_windows|meeting_pack_fact|1min.AI|True|['meeting_prep']|Meeting Prep|draft|BrowserAct,Paperguide|meeting_prep|4|step_input_prepare,step_policy_evaluate,step_artifact_save,step_memory_candidate_stage" ]]; then
...
DISPATCH_MEMORY_REVIEW_EXECUTE_JSON="$(curl -fsS -X POST "${BASE}/v1/plans/execute" "${AUTH_ARGS[@]}" "${PRINCIPAL_ARGS[@]}" -H 'content-type: application/json' \
  -d "{\"task_key\":\"stakeholder_review_dispatch_memory_candidate\",\"goal\":\"review, send, and stage stakeholder follow-up memory\",\"input_json\":{\"source_text\":\"Board context and stakeholder sensitivities.\",\"binding_id\":\"${DISPATCH_MEMORY_REVIEW_BINDING_ID}\",\"channel\":\"email\",\"recipient\":\"reviewed-memory@example.com\"}}")"
...
DISPATCH_MEMORY_REVIEW_RETURN_JSON="$(curl -fsS -X POST "${BASE}/v1/human/tasks/${DISPATCH_MEMORY_REVIEW_HUMAN_TASK_ID}/return" "${AUTH_ARGS[@]}" "${PRINCIPAL_ARGS[@]}" -H 'content-type: application/json' \
...
if [[ "${DISPATCH_MEMORY_REVIEW_CANDIDATE_FIELDS}" != "stakeholder_follow_up_fact|exec-1|${DISPATCH_MEMORY_REVIEW_DELIVERY_ID}|reviewed-memory@example.com|Reviewed stakeholder briefing with follow-up notes." ]]; then
...
HYBRID_RETRY_EXECUTE_JSON="$(curl -fsS -X POST "${BASE}/v1/plans/execute" "${AUTH_ARGS[@]}" "${PRINCIPAL_ARGS[@]}" -H 'content-type: application/json' \
  -d '{"task_key":"stakeholder_review_dispatch_retry","goal":"review and send a stakeholder briefing","input_json":{"source_text":"Board context and stakeholder sensitivities.","binding_id":"missing-review-dispatch-binding","channel":"email","recipient":"hybrid-retry@example.com"}}')"
...
HYBRID_RETRY_RETURN_JSON="$(curl -fsS -X POST "${BASE}/v1/human/tasks/${HYBRID_RETRY_HUMAN_TASK_ID}/return" "${AUTH_ARGS[@]}" "${PRINCIPAL_ARGS[@]}" -H 'content-type: application/json' \
  -d '{"operator_id":"briefing-reviewer","resolution":"ready_for_dispatch","returned_payload_json":{"final_text":"Reviewed stakeholder briefing."},"provenance_json":{"review_mode":"human"}}')"
...
HYBRID_RETRY_PENDING_AFTER_FIELDS="$(curl -fsS "${BASE}/v1/delivery/outbox/pending?limit=20" "${AUTH_ARGS[@]}" | python3 -c "import json,sys; rows=json.loads(sys.stdin.read() or '[]'); print(any((row or {}).get('recipient') == 'hybrid-retry@example.com' for row in rows))" )"
