...
SESSION_RUNTIME_FIELDS="$(python3 -c "import json,sys; body=json.loads(sys.stdin.read() or '{}'); names=[e.get('name','') for e in (body.get('events') or [])]; events=set(names); queues=body.get('queue_items') or []; steps=body.get('steps') or []; history=body.get('human_task_assignment_history') or []; artifacts=body.get('artifacts') or []; first=(artifacts[0] if artifacts else {}); order_ok=('input_prepared' in events and 'policy_decision' in events and 'policy_step_completed' in events and names.index('input_prepared') < names.index('policy_decision') < names.index('policy_step_completed')); step_lookup={str((row.get('input_json') or {}).get('plan_step_key') or ''): row for row in steps}; input_step=step_lookup.get('step_input_prepare') or {}; policy_step=step_lookup.get('step_policy_evaluate') or {}; save_step=step_lookup.get('step_artifact_save') or {}; input_id=str(input_step.get('step_id','')); policy_id=str(policy_step.get('step_id','')); projection_ok=(input_step.get('dependency_keys') == [] and input_step.get('dependency_states') == {} and input_step.get('dependency_step_ids') == {} and input_step.get('blocked_dependency_keys') == [] and input_step.get('dependencies_satisfied') is True and policy_step.get('dependency_keys') == ['step_input_prepare'] and policy_step.get('dependency_states') == {'step_input_prepare': 'completed'} and (policy_step.get('dependency_step_ids') or {}).get('step_input_prepare') == input_id and policy_step.get('blocked_dependency_keys') == [] and policy_step.get('dependencies_satisfied') is True and save_step.get('dependency_keys') == ['step_policy_evaluate'] and save_step.get('dependency_states') == {'step_policy_evaluate': 'completed'} and (save_step.get('dependency_step_ids') or {}).get('step_policy_evaluate') == policy_id and save_step.get('blocked_dependency_keys') == [] and save_step.get('dependencies_satisfied') is True); print('{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}'.format(body.get('status',''), len(steps) >= 3, len(queues) >= 3 and all((q or {}).get('state','') == 'done' for q in queues), 'input_prepared' in events, 'policy_decision' in events, 'policy_step_completed' in events, 'tool_execution_completed' in events, len(history) == 0 and order_ok, projection_ok, first.get('preview_text',''), first.get('storage_handle','')))" <<<"${SESSION_JSON}")"
if [[ "${SESSION_RUNTIME_FIELDS}" != "completed|True|True|True|True|True|True|True|True|smoke run|artifact://${ARTIFACT_ID}" ]]; then
  echo "expected initial rewrite session to complete with ordered queued input/policy events, done queue items, dependency-state projection metadata, empty human-task assignment history, and artifact envelope fields; got ${SESSION_RUNTIME_FIELDS}" >&2
  echo "${SESSION_JSON}" >&2
  fail 12 "policy contract mismatch"
fi
