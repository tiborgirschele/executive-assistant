...
PLAN_FIELDS="$(python3 -c "import json,sys; body=json.loads(sys.stdin.read() or '{}'); steps=body.get('plan',{}).get('steps') or []; prepare=(steps[0] if steps else {}); policy=(steps[1] if len(steps) > 1 else {}); save=(steps[2] if len(steps) > 2 else {}); print('{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}'.format(len(steps), prepare.get('step_key',''), prepare.get('owner',''), prepare.get('authority_class',''), policy.get('step_key',''), ','.join(policy.get('depends_on') or []), policy.get('owner',''), save.get('tool_name',''), save.get('owner',''), save.get('authority_class',''), save.get('failure_strategy','')))" <<<"${PLAN_JSON}")"
if [[ "${PLAN_FIELDS}" != "3|step_input_prepare|system|observe|step_policy_evaluate|step_input_prepare|system|artifact_repository|tool|draft|fail" ]]; then
