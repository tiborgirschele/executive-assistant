...
TASK_EXECUTE_FIELDS="$(python3 -c "import json,sys; body=json.loads(sys.stdin.read() or '{}'); print('{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}'.format(body.get('task_key',''), body.get('kind',''), body.get('deliverable_type',''), body.get('content',''), body.get('mime_type',''), body.get('preview_text',''), body.get('storage_handle',''), body.get('body_ref',''), body.get('principal_id',''), bool(body.get('artifact_id','')), bool(body.get('execution_session_id','')), body.get('structured_output_json',{}) == {} and body.get('attachments_json',{}) == {}))" <<<"${TASK_EXECUTE_JSON}")"
if [[ "${TASK_EXECUTE_FIELDS}" != "stakeholder_briefing|stakeholder_briefing|stakeholder_briefing|Board context and stakeholder sensitivities.|text/plain|Board context and stakeholder sensitivities.|artifact://${TASK_EXECUTE_ARTIFACT_ID}|artifact://${TASK_EXECUTE_ARTIFACT_ID}|${PRINCIPAL_ID}|True|True|True" ]]; then
...
TASK_EXECUTE_SESSION_FIELDS="$(python3 -c "import json,sys; body=json.loads(sys.stdin.read() or '{}'); artifacts=body.get('artifacts') or []; steps=body.get('steps') or []; events=body.get('events') or []; first=(artifacts[0] if artifacts else {}); prepare=(steps[0] if steps else {}); policy=(steps[1] if len(steps) > 1 else {}); save=(steps[2] if len(steps) > 2 else {}); plan_event=next((event for event in events if (event or {}).get('name') == 'plan_compiled'), {}); semantics=(plan_event.get('payload',{}) or {}).get('step_semantics') or []; first_semantics=(semantics[0] if semantics else {}); prepare_input=prepare.get('input_json',{}) or {}; parent_ok=(prepare.get('parent_step_id') is None and policy.get('parent_step_id') == prepare.get('step_id') and save.get('parent_step_id') == policy.get('step_id')); print('{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}'.format(body.get('intent_task_type',''), body.get('status',''), len(steps), first.get('kind',''), first.get('task_key',''), first.get('deliverable_type',''), any((event or {}).get('name') == 'plan_compiled' for event in events), prepare_input.get('owner',''), prepare_input.get('authority_class',''), prepare_input.get('timeout_budget_seconds',''), (save.get('input_json',{}) or {}).get('owner',''), (save.get('input_json',{}) or {}).get('failure_strategy',''), first_semantics.get('owner',''), first_semantics.get('timeout_budget_seconds',''), first.get('mime_type',''), first.get('preview_text',''), first.get('storage_handle',''), str(first.get('body_ref','')).startswith('file://'), first.get('principal_id',''), parent_ok, prepare_input.get('channel',''), prepare_input.get('stakeholder_ref',''), ','.join(prepare_input.get('context_refs') or [])))" <<<"${TASK_EXECUTE_SESSION_JSON}")"
if [[ "${TASK_EXECUTE_SESSION_FIELDS}" != "stakeholder_briefing|completed|3|stakeholder_briefing|stakeholder_briefing|stakeholder_briefing|True|system|observe|30|tool|fail|system|30|text/plain|Board context and stakeholder sensitivities.|artifact://${TASK_EXECUTE_ARTIFACT_ID}|True|${PRINCIPAL_ID}|True|email|alex-exec|thread:board-prep,memory:item:stakeholder-brief" ]]; then
...
TASK_EXECUTE_ARTIFACT_FIELDS="$(python3 -c "import json,sys; body=json.loads(sys.stdin.read() or '{}'); print('{}|{}|{}|{}|{}|{}|{}|{}|{}'.format(body.get('task_key',''), body.get('kind',''), body.get('deliverable_type',''), body.get('execution_session_id',''), body.get('mime_type',''), body.get('preview_text',''), body.get('storage_handle',''), str(body.get('body_ref','')).startswith('file://'), body.get('principal_id','')))" <<<"${TASK_EXECUTE_ARTIFACT_JSON}")"
if [[ "${TASK_EXECUTE_ARTIFACT_FIELDS}" != "stakeholder_briefing|stakeholder_briefing|stakeholder_briefing|${TASK_EXECUTE_SESSION_ID}|text/plain|Board context and stakeholder sensitivities.|artifact://${TASK_EXECUTE_ARTIFACT_ID}|True|${PRINCIPAL_ID}" ]]; then
