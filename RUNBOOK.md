...
| GET | `/v1/rewrite/artifacts/{artifact_id}` | `200` | `404 artifact_not_found`, `403 principal_scope_mismatch` (returns artifact content plus originating `task_key`/`deliverable_type`, `preview_text`, and `storage_handle`) |
...
- Rewrite artifact reads and inline session artifact rows now also expose `preview_text` and `storage_handle`, so artifact consumers can start treating them as metadata-plus-handle envelopes without dropping the current inline `content` contract.
