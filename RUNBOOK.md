...
- Rewrite artifact reads, inline session artifact rows, and generic task execution responses now also expose explicit `principal_id` ownership alongside `preview_text` and `storage_handle`, so artifact consumers can start treating them as metadata-plus-handle envelopes without dropping the current inline `content` contract.
