...
- `POST /v1/tools/execute` also exposes `browseract.extract_account_facts`, which self-heals its built-in registry definition, resolves BrowserAct-backed account facts from a principal-scoped connector binding, and can optionally call a configured live BrowserAct run URL when the binding metadata supplies one.
