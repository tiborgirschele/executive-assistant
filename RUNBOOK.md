...
| GET | `/v1/human/tasks/priority-summary` | `200` | validation `422`, `403 principal_scope_mismatch` (supports `status`, `role_required`, `assigned_operator_id`, `assignment_state`, and `overdue_only`) |
| GET | `/v1/human/tasks` | `200` | validation `422`, `403 principal_scope_mismatch` (supports `role_required`, `priority`, `assigned_operator_id`, `assignment_state`, `overdue_only`, and `sort=created_asc|created_desc|last_transition_desc|priority_desc_created_asc|sla_due_at_asc|sla_due_at_asc_last_transition_desc`) |
| GET | `/v1/human/tasks/backlog` | `200` | validation `422` (supports `priority`, `assignment_state` and `sort=created_asc|created_desc|last_transition_desc|priority_desc_created_asc|sla_due_at_asc|sla_due_at_asc_last_transition_desc`) |
| GET | `/v1/human/tasks/unassigned` | `200` | validation `422` (supports `priority` and `sort=created_asc|created_desc|last_transition_desc|priority_desc_created_asc|sla_due_at_asc|sla_due_at_asc_last_transition_desc`) |
| GET | `/v1/human/tasks/mine` | `200` | validation `422` (supports `priority` and `sort=created_asc|created_desc|last_transition_desc|priority_desc_created_asc|sla_due_at_asc|sla_due_at_asc_last_transition_desc`) |
...
| GET | `/v1/human/tasks/{human_task_id}/assignment-history` | `200` | `404 human_task_not_found`, validation `422` (supports `event_name`, `assigned_operator_id`, and `assigned_by_actor_id`) |
...
- `GET /v1/human/tasks/{human_task_id}/assignment-history` also accepts `event_name`, `assigned_operator_id`, and `assigned_by_actor_id` to narrow that transition chain to specific reassignment, claim, or return views for operator tooling.
- `GET /v1/rewrite/sessions/{session_id}` now also includes `human_task_assignment_history`, mirroring those task-scoped ownership transitions inline with the broader session detail payload for one-fetch operator views.
